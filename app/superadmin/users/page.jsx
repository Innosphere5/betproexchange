...
export default function SuperAdminUsers() {
  ...
  const handleClearReport = async () => {
    let confirmMsg = `Are you sure you want to clear all report data for ${selectedDate}? This cannot be undone.`;
    if (reportPeriod === 'monthly') confirmMsg = `Are you sure you want to clear all report data for ${selectedMonth}?`;
    else if (reportPeriod === 'yearly') confirmMsg = `Are you sure you want to clear all report data for Year ${selectedYear}?`;
    else if (reportPeriod === 'range') confirmMsg = `Are you sure you want to clear all report data from ${startDate} to ${endDate}?`;
    ...
    const token = getAuthToken();
    try {
      let url = `${getApiUrl()}/api/admin/clear-daily-report?reportType=${reportPeriod}`;
      if (reportPeriod === 'daily') url += `&date=${selectedDate}`;
      else if (reportPeriod === 'monthly') url += `&month=${selectedMonth}`;
      else if (reportPeriod === 'yearly') url += `&year=${selectedYear}`;
      else if (reportPeriod === 'range') url += `&startDate=${startDate}&endDate=${endDate}`;
  ...
  const handleBalanceUpdate = async (e, mode, tab) => {
    ...
    console.log(`[DEBUG] SuperAdmin Balance Update Request:`, {
      endpoint,
      mode,
          ...
          <div className="bg-white border border-gray-300 shadow-sm rounded-sm overflow-hidden flex flex-col">
            <div className="bg-[#f2f2f2] border-b border-gray-300 px-3 py-2 font-bold text-gray-800 text-[13px] flex items-center justify-between">
              ...
              {breadcrumbs && breadcrumbs.length > 1 && (
                <button
                  ...
                  className="bg-[#1abc9c] hover:bg-[#16a085] text-white text-[11px] font-bold px-2 py-0.5 rounded transition-all flex items-center gap-1 shadow-sm"
                >
                  ← Up Level
                </button>
              )}
                ...
                {breadcrumbs.map((b, idx) => {
                  ...
                  return (
                    <React.Fragment key={b._id || b.username}>
                      ...
                      {isLast ? (
                        <span className="text-[#1abc9c] font-bold underline bg-teal-50 px-2 py-0.5 rounded">
            ...
            <div className="px-4 py-3 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-3">
              <div className="flex items-center gap-2">
                ...
                <Link href="/superadmin/account-ledger" className="bg-[#1abc9c] hover:bg-[#16a085] text-white px-3 py-1.5 text-sm font-semibold rounded-sm flex items-center gap-1">
                  <BookOpen size={16} />
                  Account Ledger
                </Link>
              </div>
              ...
              <table className="w-full text-sm text-left">
                <thead>
                  <tr className="bg-white border-b border-gray-200">
                    ...
                    <th className="px-4 py-2.5 font-bold text-gray-800 border-r border-gray-200 text-[#1abc9c]">Available Balance</th>
