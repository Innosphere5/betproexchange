...
const navItems = [
  { label: "Dashboard", href: "/supermaster/dashboard", icon: <LayoutDashboard size={18} className="opacity-80" /> },
  { label: "Users (Bettors)", href: "/supermaster/users", icon: <Users size={18} className="opacity-80" /> },
  { label: "Account Ledger", href: "/supermaster/account-ledger", icon: <BookOpen size={18} className="opacity-80" /> },
  { label: "Current Position", href: "/supermaster/current-position", icon: <Filter size={18} className="opacity-80" /> },
  { label: "Reports", href: "/supermaster/reports", icon: <FileText size={18} className="opacity-80" /> }
];

export default function SuperMasterSidebar({ isOpen, setIsOpen }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between h-14 px-4 bg-[#1a252f] shrink-0 border-b border-[#1f2d3a]">
        <div className="text-white font-bold text-lg tracking-wide invisible lg:visible uppercase text-[#8e44ad]">SUPER MASTER</div>
                ...
                {hasChildren ? (
                  ...
                  <Link
                    ...
                    className={`flex items-center gap-3 px-5 py-3 text-sm font-medium transition-colors border-l-[3px] 
                      ${isActive
                         ? 'bg-[#1a252f] text-white border-[#8e44ad] font-bold'
