...
export default function NotificationPopup({ notification, onClose }) {
  ...
  const isRefund = notification.status === 'CANCELLED';
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 animate-slide-up">
      <div className={`relative px-5 py-4 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border overflow-hidden min-w-[320px] 
        ${isWin ? 'bg-[#1e293b] border-green-500/30' : isRefund ? 'bg-[#1e293b] border-blue-500/30' : 'bg-[#1e293b] border-red-500/30'}
      `}>
        {/* Glow effect */}
        <div className={`absolute top-0 right-0 w-full h-1 
          ${isWin ? 'bg-gradient-to-r from-green-600 to-green-400' : isRefund ? 'bg-gradient-to-r from-blue-600 to-blue-400' : 'bg-gradient-to-r from-red-600 to-red-400'}
        `}></div>
        ...
        <div className="flex items-start gap-4 mt-1">
          <div className={`mt-0.5 p-2 rounded-full shrink-0
            ${isWin ? 'bg-green-500/10 text-green-400' : isRefund ? 'bg-blue-500/10 text-blue-400' : 'bg-red-500/10 text-red-400'}
          `}>
            {isWin ? <CheckCircle size={22} /> : isRefund ? <CheckCircle size={22} className="text-blue-400" /> : <XCircle size={22} />}
          </div>
          
          <div className="flex flex-col">
            <h3 className={`text-base font-bold tracking-tight
              ${isWin ? 'text-green-400' : isRefund ? 'text-blue-400' : 'text-red-400'}
            `}>
              {isWin ? 'Bet Won!' : isRefund ? 'Bet Refunded' : 'Bet Lost'}
            </h3>
